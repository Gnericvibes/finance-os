...
export async function createFinancialProfile(
  ...
  const profile = await db.financialProfile.create({
    data: {
      ...
      repaymentAmount:
        input.repaymentAmount ?? 0,

      mainFinancialGoal:
        input.mainFinancialGoal,

      emergencySavingsGoal:
        input.emergencySavingsGoal ?? 0,
  ...
  const blueprint =
    PFOSEngine.generateBlueprint({
      ...
      miscellaneousExpenses:
        input.miscellaneousExpenses ?? 0,

      dependentsCount:
        input.dependentsCount ?? 0,
    });

    await db.householdExpenseProfile.upsert({
      where: {
        userId: input.userId,
      },
    ...
    await db.financialBlueprint.updateMany({
      where: {
        userId: input.userId,
        isActive: true,
      },
  ...
  const latestBlueprint = await db.financialBlueprint.findFirst({
    where: {
      userId: input.userId,
    },
    orderBy: {
