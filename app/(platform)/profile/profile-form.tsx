...
export function ProfileForm({ userId, profile }: ProfileFormProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Checkboxes: set explicit true/false since unchecked boxes send nothing
  ...
  return (
    <section className="rounded-3xl border border-zinc-800 bg-zinc-950 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button
          type="submit"
          className="w-full rounded-xl bg-white px-6 py-3 font-semibold text-black hover:bg-zinc-200 transition-colors"
        >
          Save Changes
        </button>
      </form>
